...
func TestRegisterHandler(t *testing.T) {
	...
	amqpMock.EXPECT().ExchangeDeclare(constants.MatchedRideExchange, "fanout", true, false, false, false, nil).
		Return(nil).AnyTimes()
	amqpMock.EXPECT().QueueDeclare(gomock.Any(), false, false, true, false, nil).
		Return(amqp091.Queue{}, nil).AnyTimes()
	amqpMock.EXPECT().QueueBind(gomock.Any(), gomock.Any(), constants.MatchedRideExchange, gomock.Any(), nil).
		Return(nil).AnyTimes()
	...
	expectedRoutes := gin.RoutesInfo{
		{
			Method:  "GET",
			Path:    "/ws/riders",
			Handler: "nebeng-jek/internal/riders/handler.(*ridersHandler).RiderWebsocket-fm",
		},
	}
